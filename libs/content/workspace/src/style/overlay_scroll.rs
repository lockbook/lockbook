...
/// How long the bar stays grabable after scroll/leave. Independent of
/// chrome slide (`ANIM_MAX_SECS`) — yoking them made the thumb vanish
/// before a pointer could reach it.
...
/// Gap from the scroller's right edge. Affine bars and egui `ScrollArea`
/// both use this so the pill sits in the same place.
pub const BAR_INSET: f32 = 2.0;

/// Corner radius for a 6px pill.
const BAR_RADIUS: f32 = 3.0;

/// Thumb on the content; track only while the bar is held, as a whisper of ink
/// (not paper — egui's built-in track uses `extreme_bg` and reads white).
const THUMB_IDLE: f32 = 0.50;
const THUMB_INTERACT: f32 = 0.85;
const TRACK_INTERACT: f32 = 0.08;
...
/// Whether the overlay would paint this frame (recent scroll, or thumb held).
/// Custom bars use this to skip hit-testing a hidden thumb so it cannot
/// steal body/handle drags.
pub fn is_shown(ui: &Ui, id: Id) -> bool {
...
fn apply_egui_style(ui: &mut Ui, show: bool) {
    let ink = ui.ctx().get_lb_theme().neutral_fg();
    {
        ...
        for w in [
            ...
            w.corner_radius = egui::CornerRadius::same(BAR_RADIUS as u8);
            w.expansion = 0.0;
    ...
    scroll.bar_outer_margin = BAR_INSET;
    scroll.dormant_handle_opacity = 0.0;
    scroll.dormant_background_opacity = 0.0;
    scroll.foreground_color = true;

    if show {
        // Same numbers as [`paint`]. Width 0 when hidden so a faded egui
        // thumb cannot steal hits (affine bars skip `interact` when idle).
        // Track opacity stays 0: egui fills `extreme_bg` (white in light).
        scroll.floating_width = BAR_WIDTH;
        scroll.bar_width = BAR_WIDTH;
        scroll.active_handle_opacity = THUMB_IDLE;
        scroll.active_background_opacity = 0.0;
        scroll.interact_handle_opacity = THUMB_INTERACT;
...
fn track_ink(ui: &Ui) -> egui::Color32 {
    ui.ctx()
        .get_lb_theme()
        .neutral_fg()
        .gamma_multiply(TRACK_INTERACT)
}

/// Overlay thumb; faint ink track only while the bar is held.
pub fn paint(ui: &Ui, track: Rect, thumb: Rect, interact: bool) {
    let t = ui.ctx().get_lb_theme();
    let ink = t.neutral_fg();
    if interact {
        ui.painter().rect_filled(track, BAR_RADIUS, track_ink(ui));
    }
    let thumb_a = if interact { THUMB_INTERACT } else { THUMB_IDLE };
    ui.painter()
...
pub fn with_overlay_scroll<R>(ui: &mut Ui, id: Id, f: impl FnOnce(&mut Ui) -> (R, f32, Id)) -> R {
    ui.scope(|ui| {
        ...
        note_offset(ui, id, offset_y);
        let held = bar_held(ui.ctx(), scroll_area_id);
        note_bar_interaction(ui, id, held);
        if held {
            let track = Rect::from_min_max(
                egui::pos2(outer.max.x - BAR_INSET - BAR_WIDTH, outer.min.y),
                egui::pos2(outer.max.x - BAR_INSET, outer.max.y),
            );
            ui.painter().rect_filled(track, BAR_RADIUS, track_ink(ui));
        }
        result
