...
impl<'ast> MarkdownPlusPlus {
    ...
    pub fn calc_paragraphs(&self, node: &'ast AstNode<'ast>) -> Paragraphs {
        ...
        for descendant in node.descendants() {
            let sourcepos = descendant.data.borrow().sourcepos;
            ...
            match &descendant.data.borrow().value {
                ...
                NodeValue::CodeBlock(NodeCodeBlock {
                    ...
                    let info_length = RelByteOffset(info.len());
                    let code_length = RelByteOffset(literal.len());
                    ...
                    if *fenced {
                        ...
                        if sourcepos.end.line - sourcepos.start.line > 0 {
                            let first_code_line_idx = info_line_idx + 1;
                            let first_code_line_range =
                ...
                NodeValue::Heading(NodeHeading { level, setext }) => {
                    if !setext {
                        ...
                        result.push((range.start(), text_end_char))
                    }
                }
                ...
                NodeValue::TableCell => result.push(range), // todo: split lines
    ...
    pub fn calc_lines(&self) -> Lines {
        todo!() // reference line wrap
    }

    pub fn calc_text(&self) -> Text {
        todo!() // reference capture
    }
}
