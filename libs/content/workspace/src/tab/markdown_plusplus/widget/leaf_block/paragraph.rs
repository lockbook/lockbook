...
impl<'ast> MarkdownPlusPlus {
    ...
    pub fn show_paragraph(&mut self, ui: &mut Ui, node: &'ast AstNode<'ast>, mut top_left: Pos2) {
        ...
        for line in self.node_lines(node).iter() {
            ...
            top_left.y += self.height_paragraph_line(node, line);
            top_left.y += BLOCK_SPACING;
        }
    }
    ...
    pub fn show_paragraph_line(
        ...
        self.show_line_prefix(ui, parent, line, top_left, line_height, wrap.row_height); // todo quoted empty paragraph broken
