...
impl Chat {
    pub fn new(
        ...
        files: Arc<RwLock<FileCache>>, doc_index: DocIndex, tasks: TaskManager, core: &Lb,
        cfg: WsPersistentStore, tabs: Arc<Mutex<TabBridge>>, images: ImageCache,
    ) -> Self {
        ...
        composer.renderer.doc_index = doc_index.clone();
    ...
    pub(super) fn md_label(&mut self, id: Uuid) -> &mut MdLabel {
        ...
        let label = self.labels.get_mut(&id).expect("just inserted");
        label.renderer.files = Arc::clone(&self.files);
        label.renderer.doc_index = self.doc_index.clone();
        label.renderer.link_resolver =
            Box::new(FileCacheLinkResolver::new(Arc::clone(&self.files), self.id));
    ...
    fn spawn_tools(&mut self, parent: Uuid, calls: Vec<grok::ToolCall>) {
        ...
        thread::spawn(move || {
            ...
            for c in calls {
                ...
                let result = dispatch_client_tool(
                    &core, chat_id, &c.name, &c.args, &tabs, &cfg, false, &files, &doc_index,
                    &tasks,
                );
                out.push((c, result));
            }
...
pub(super) fn dispatch_client_tool(
    ...
    cfg: &WsPersistentStore, on_call: bool, files: &Arc<RwLock<FileCache>>, doc_index: &DocIndex,
    tasks: &TaskManager,
    ...
    match name {
        ...
        "broken_links" => {
            ...
            tools::broken_links(core, chat_id, &files, doc_index, args)
                .map(tools::ClientToolOut::text)
        }
        "resolve_link" => {
            let files = files.read().unwrap();
            tools::resolve_link(core, chat_id, &files, args).map(tools::ClientToolOut::text)
        }
        "unreferenced_images" => {
            let files = files.read().unwrap();
            tools::unreferenced_images(core, chat_id, &files, doc_index, args)
                .map(tools::ClientToolOut::text)
        }
        "rename" => {
            let files = files.read().unwrap();
            tools::rename(core, chat_id, &files, doc_index, tasks, args)
                .map(tools::ClientToolOut::text)
        }
        "move" => {
            let files = files.read().unwrap();
            tools::move_file(core, chat_id, &files, doc_index, tasks, args)
                .map(tools::ClientToolOut::text)
        }
