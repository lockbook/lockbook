...
impl<'ast, E: EmbedResolver, L: LinkResolver> MdEdit<E, L> {
    ...
    pub fn apply_fold(
        ...
        if unapply {
            ...
            if let Some(fold) = self.renderer.fold(node) {
                self.event.internal_events.push(Event::Replace {
                    region: self.renderer.node_range(fold).into(),
                    text: "".into(),
            ...
            if let Some(foldable) = self.renderer.foldable(node) {
                self.event.internal_events.push(Event::Replace {
                    region: self.renderer.node_range(foldable).end().into_range().into(),
                    text: FOLD_TAG.into(),
                    advance_cursor: false,
                });
