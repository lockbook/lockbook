...
impl<'ast> Editor {
    pub fn height_table_cell(&self, node: &'ast AstNode<'ast>) -> f32 {
        let width = self.width(node) - 2.0 * self.visuals.block_padding;
        ...
        for descendant in node.descendants() {
            if let NodeValue::Image(node_link) = &descendant.data.borrow().value {
                ...
                images_height += self.visuals.block_spacing;
            }
        }

        wrap.offset += self.inline_children_span(node, &wrap, node_line);

        self.visuals.block_padding + images_height + wrap.height() + self.visuals.block_padding
    }
