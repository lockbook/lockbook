...
/// Draws the popup frame and per-row highlight backgrounds for a completion popup.
/// Rows are uniform height (`COMPLETION_ROW_HEIGHT`). `selected` is the
/// keyboard-highlighted index; `hover_pos` is the current pointer position.
