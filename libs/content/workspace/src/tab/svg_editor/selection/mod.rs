...
use self::{
    rect::{SelectionRect, SCALE_BRUSH_SIZE},
    scale::{scale_from_center, snap_scale},
    translate::{detect_translation, end_translation, save_translate},
};
...
impl Selection {
    pub fn handle_input(
        ...
        for el in self.selected_elements.iter_mut() {
            ...
            let selection_rect = SelectionRect::new(path, working_rect).unwrap();
            selection_rect.show(ui);
            let mut suggested_icon = selection_rect.get_cursor_icon(path, pos);
            ...
            if ui.input(|r| r.pointer.primary_released()) {
                ...
                if matches!(self.current_op, SelectionOperation::Idle) {
                    if let Some(r) = &mut suggested_icon {
                ...
                match self.current_op {
                    ...
                    | SelectionOperation::SouthScale => {
                        let icon = snap_scale(pos, el, buffer);
                        if let Some(c) = icon {
            ...
            } else if let Some(r) = suggested_icon {
                ui.output_mut(|w| w.cursor_icon = r.cursor_icon);
            }
            ...
            if is_scaling_down || is_scaling_up {
                scale_from_center(scale_factor, el, buffer);
                transform_origin_dirty = true;
                history_dirty = true;
            }
        }
        ...
        if transform_origin_dirty {
            end_translation(buffer, &mut self.selected_elements, pos, history_dirty);
        }
