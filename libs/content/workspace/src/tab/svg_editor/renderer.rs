...
pub struct Renderer {
    mesh_tx: Sender<egui::Mesh>,
    mesh_rx: Receiver<egui::Mesh>,
}
...
impl Renderer {
    pub fn new() -> Self {
        ...
        Self { mesh_rx, mesh_tx }
    }
    pub fn render_svg(&self, ui: &mut egui::Ui, buffer: &mut Buffer, inner_rect: egui::Rect) {
        let painter = ui
                    ...
                    parser::Element::Path(p) => {
                        ...
                        if let Some(stroke) = p.stroke {
                            ...
                            while let Some(seg) = p.data.get_segment(i) {
                                let pressure =
                                    if let Some(ref p) = p.pressure { p.get(i) } else { None };

                                let thickness =
                                    stroke.width * master_transform.sx * pressure.unwrap_or(&1.0);
