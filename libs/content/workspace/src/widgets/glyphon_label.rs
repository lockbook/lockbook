...
impl<'a> GlyphonLabel<'a> {
    ...
    fn shape_buffer(
        ...
        let font_size_px = font_size * ppi;
        let line_height_px = line_height * ppi;
        let max_width_px = max_width * ppi;
        ...
        let metrics_line_height = buf.metrics().line_height;
        let (width, lines) = buf
            .layout_runs()
            .fold((0.0f32, 0u32), |(w, n), r| (w.max(r.line_w), n + 1));
        let size = egui::vec2(width / ppi, lines as f32 * metrics_line_height / ppi);
        drop(buf);
