...
public struct Status {
    ...
    init(_ status: LbStatus) {
        ...
        self.dirtyLocally = Array(UnsafeBufferPointer(start: status.dirty_locally.ids, count: Int(status.pushing_files.len))).toUUIDs()
        self.pushingFiles = Array(UnsafeBufferPointer(start: status.pushing_files.ids, count: Int(status.pushing_files.len))).toUUIDs()
        self.pullingFiles = Array(UnsafeBufferPointer(start: status.pulling_files.ids, count: Int(status.pushing_files.len))).toUUIDs()
