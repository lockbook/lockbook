...
async fn read_count() {
    ...
    let document1 = core.create_at_path("hello1.md").await.unwrap();
    for _ in 0..10 {
        core.read_document(document1.id, false).await.unwrap();
    }

    let document2 = core.create_at_path("hello2.md").await.unwrap();
    for _ in 0..20 {
        core.read_document(document2.id, false).await.unwrap();
    }
...
async fn read_count_multiple_docs() {
    ...
    let document3 = core.create_at_path("hello3.md").await.unwrap();
    for _ in 0..100 {
        core.read_document(document3.id, false).await.unwrap();
    }
