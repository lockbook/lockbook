...
async fn two_cycle() {
    ...
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    move_by_path(&mut c2, "/b/", "/a/").await.unwrap();
...
async fn three_cycle_one_move_reverted() {
    ...
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    move_by_path(&mut c1, "/b/", "/c/").await.unwrap();
    move_by_path(&mut c2, "/c/", "/a/").await.unwrap();
...
async fn three_cycle_two_moves_reverted() {
    ...
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    move_by_path(&mut c2, "/b/", "/c/").await.unwrap();
    move_by_path(&mut c2, "/c/", "/a/").await.unwrap();
...
async fn four_cycle_one_move_reverted() {
    ...
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    move_by_path(&mut c1, "/b/", "/c/").await.unwrap();
    move_by_path(&mut c1, "/c/", "/d/").await.unwrap();
    move_by_path(&mut c2, "/d/", "/a/").await.unwrap();
...
async fn four_cycle_two_moves_reverted_adjacent() {
    ...
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    move_by_path(&mut c1, "/b/", "/c/").await.unwrap();
    move_by_path(&mut c2, "/c/", "/d/").await.unwrap();
    move_by_path(&mut c2, "/d/", "/a/").await.unwrap();
...
async fn two_cycle_with_renames_first_device() {
    ...
    rename_path(&mut c1, "/a/", "a2").await.unwrap();
    rename_path(&mut c1, "/b/", "b2").await.unwrap();

    move_by_path(&mut c1, "/a2/", "/b2/").await.unwrap();
    move_by_path(&mut c2, "/b/", "/a/").await.unwrap();
...
async fn three_cycle_one_move_reverted_with_renames_first_device() {
    ...
    rename_path(&mut c1, "/a/", "a2").await.unwrap();
    rename_path(&mut c1, "/b/", "b2").await.unwrap();
    rename_path(&mut c1, "/c/", "c2").await.unwrap();

    move_by_path(&mut c1, "/a2/", "/b2/").await.unwrap();
    move_by_path(&mut c1, "/b2/", "/c2/").await.unwrap();
    move_by_path(&mut c2, "/c/", "/a/").await.unwrap();
...
async fn four_cycle_one_move_reverted_with_renames_first_device() {
    ...
    rename_path(&mut c1, "/b/", "b2").await.unwrap();
    rename_path(&mut c1, "/c/", "c2").await.unwrap();
    rename_path(&mut c1, "/d/", "d2").await.unwrap();

    move_by_path(&mut c1, "/a2/", "/b2/").await.unwrap();
    move_by_path(&mut c1, "/b2/", "/c2/").await.unwrap();
    move_by_path(&mut c1, "/c2/", "/d2/").await.unwrap();
    move_by_path(&mut c2, "/d/", "/a/").await.unwrap();
...
async fn two_cycle_with_renames_second_device() {
    ...
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    rename_path(&mut c2, "/a/", "a2").await.unwrap();
    rename_path(&mut c2, "/b/", "b2").await.unwrap();
    move_by_path(&mut c2, "/b2/", "/a2/").await.unwrap();
...
async fn three_cycle_one_move_reverted_with_renames_second_device() {
    ...
    rename_path(&mut c2, "/a/", "a2").await.unwrap();
    rename_path(&mut c2, "/b/", "b2").await.unwrap();
    rename_path(&mut c2, "/c/", "c2").await.unwrap();
    move_by_path(&mut c2, "/c2/", "/a2/").await.unwrap();
...
async fn three_cycle_two_moves_reverted_with_renames_second_device() {
    ...
    rename_path(&mut c2, "/b/", "b2").await.unwrap();
    rename_path(&mut c2, "/c/", "c2").await.unwrap();
    move_by_path(&mut c2, "/b2/", "/c2/").await.unwrap();
    move_by_path(&mut c2, "/c2/", "/a2/").await.unwrap();
...
async fn four_cycle_one_move_reverted_with_renames_second_device() {
    ...
    rename_path(&mut c2, "/b/", "b2").await.unwrap();
    rename_path(&mut c2, "/c/", "c2").await.unwrap();
    rename_path(&mut c2, "/d/", "d2").await.unwrap();

    move_by_path(&mut c2, "/d2/", "/a2/").await.unwrap();
...
async fn four_cycle_two_moves_reverted_adjacent_with_renames_second_device() {
    ...
    move_by_path(&mut c2, "/c2/", "/d2/").await.unwrap();
    move_by_path(&mut c2, "/d2/", "/a2/").await.unwrap();
...
async fn two_cycle_with_deletes_first_device() {
    ...
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    move_by_path(&mut c2, "/b/", "/a/").await.unwrap();
    delete_path(&mut c1, "/b/").await.unwrap();
...
async fn three_cycle_one_move_reverted_with_deletes_first_device() {
    ...
    move_by_path(&mut c1, "/b/", "/c/").await.unwrap();
    move_by_path(&mut c2, "/c/", "/a/").await.unwrap();
    delete_path(&mut c1, "/c/").await.unwrap();
...
async fn four_cycle_one_move_reverted_with_deletes_first_device() {
    ...
    move_by_path(&mut c1, "/c/", "/d/").await.unwrap();
    move_by_path(&mut c2, "/d/", "/a/").await.unwrap();
    delete_path(&mut c1, "/d/").await.unwrap();
...
async fn two_cycle_with_deletes_second_device() {
    ...
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    move_by_path(&mut c2, "/b/", "/a/").await.unwrap();
    delete_path(&mut c2, "/a/").await.unwrap();
...
async fn three_cycle_one_move_reverted_with_deletes_second_device() {
    ...
    move_by_path(&mut c2, "/c/", "/a/").await.unwrap();
    delete_path(&mut c2, "/a/").await.unwrap();
    delete_path(&mut c2, "/b/").await.unwrap();
...
async fn four_cycle_one_move_reverted_with_deletes_second_device() {
    ...
    delete_path(&mut c2, "/a/").await.unwrap();
    delete_path(&mut c2, "/b/").await.unwrap();
    delete_path(&mut c2, "/c/").await.unwrap();
...
async fn move_two_cycle_with_modified_document_children() {
    ...
    write_path(&mut c1, "/a/child", b"document content")
        .await
        .unwrap();
    move_by_path(&mut c1, "/a/", "/b/").await.unwrap();
    move_by_path(&mut c2, "/b/", "/a/").await.unwrap();
