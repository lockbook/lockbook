...
impl Buffer {
    ...
    pub fn merge(base: &[u8], local: &[u8], remote: &[u8]) -> Vec<u8> {
        ...
        // add any remote messages not present in base (i.e. new since last common state)
        ...
        // remove any messages present in base but deleted from remote
        // (not applicable for MVP since messages are immutable, but correct to handle)
        local
            .messages
