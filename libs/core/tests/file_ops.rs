...
fn rename_not_found() {
    ...
    assert_matches!(result.unwrap_err().kind, LbErrorKind::FileNonexistent);
}
...
fn rename_not_root() {
    ...
    assert_matches!(result.unwrap_err().kind, LbErrorKind::RootModificationInvalid);
}
...
fn apply_rename_invalid_name() {
    ...
    assert_matches!(
        ...
        LbErrorKind::FileNameContainsSlash
    );
}
...
fn name_taken() {
    ...
    assert_matches!(core.rename_file(id, "doc1.md").unwrap_err().kind, LbErrorKind::PathTaken);
}
...
fn name_empty() {
    ...
    core.create_at_path("doc1.md").unwrap();
    let id = core.create_at_path("doc2.md").unwrap().id;
    assert_matches!(core.rename_file(id, "").unwrap_err().kind, LbErrorKind::FileNameEmpty);
}
...
fn mv_not_found_parent() {
    ...
    assert_matches!(
        ...
        LbErrorKind::FileParentNonexistent
    );
}
...
fn move_parent_document() {
    ...
    assert_matches!(core.move_file(id, target).unwrap_err().kind, LbErrorKind::FileNotFolder);
}
...
fn move_root() {
    ...
    assert_matches!(
        core.move_file(core.get_root().unwrap().id, id)
            .unwrap_err()
            .kind,
        LbErrorKind::RootModificationInvalid
    );
}
...
fn move_path_conflict() {
    ...
    assert_matches!(core.move_file(src, dest).unwrap_err().kind, LbErrorKind::PathTaken);
}
...
fn folder_into_self() {
    ...
    assert_matches!(core.move_file(src, dest).unwrap_err().kind, LbErrorKind::FolderMovedIntoSelf);
}
