...
fn create_at_path_path_contains_empty_file_name() {
    ...
    assert_matches!(result.unwrap_err().kind, LbErrorKind::PathContainsEmptyFileName);
}
...
fn create_at_path_path_taken() {
    ...
    assert_matches!(result.unwrap_err().kind, LbErrorKind::PathTaken);
}
...
fn create_at_path_not_folder() {
    ...
    assert_matches!(result.unwrap_err().kind, LbErrorKind::FileNotFolder);
}
