...
fn test_create_delete_read() {
    ...
    let id = core.create_at_path("test.md").unwrap().id;
    core.delete_file(id).unwrap();
    assert_matches!(core.read_document(id).unwrap_err().kind, LbErrorKind::FileNonexistent);
}
...
fn test_create_parent_delete_parent_read_doc() {
    ...
    assert_matches!(core.read_document(doc.id).unwrap_err().kind, LbErrorKind::FileNonexistent);
}
