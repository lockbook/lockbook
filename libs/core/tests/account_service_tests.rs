...
fn create_account_invalid_url() {
    ...
    assert!(matches!(result.unwrap_err().kind, LbErrorKind::ServerUnreachable))
}
...
fn create_account_username_taken() {
    ...
    assert!(
        matches!(err.kind, LbErrorKind::UsernameTaken),
...
fn create_account_invalid_username() {
    ...
    for &uname in &invalid_unames {
        ...
        assert!(
            matches!(err.kind, LbErrorKind::UsernameInvalid),
...
fn import_account_corrupted() {
    ...
    assert!(matches!(
        core.import_account("clearly a bad account string")
            .unwrap_err()
            .kind,
        LbErrorKind::AccountStringCorrupted
    ));
}
...
fn import_account_public_key_mismatch() {
    ...
    assert!(matches!(
        ...
        LbErrorKind::UsernamePublicKeyMismatch
    ));
}
