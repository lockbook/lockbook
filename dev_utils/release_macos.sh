...
if [ "$ASCPW" ]; then
    ...
    cd "$core"
    touch src/lib.rs
    API_URL="http://api.lockbook.app:8000" make lib_c_for_swift

    echo "Archiving Lockbook (macOS)"
    xcodebuild -workspace "$apple"/Lockbook.xcworkspace \
              -scheme Lockbook\ \(macOS\) \
              -sdk macos \
              -configuration Release \
              -archivePath "$apple"/build/Lockbook-macOS.xcarchive \
              archive

    echo "Export Lockbook (macOS)"
    xcodebuild \
