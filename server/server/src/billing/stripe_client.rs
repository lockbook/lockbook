...
#[derive(Clone)]
pub struct NopStripeClient {}

#[async_trait]
impl StripeClient for NopStripeClient {
    async fn create_customer(
        &self, _customer_name: String, _payment_method_idd: stripe::PaymentMethodId,
    ) -> Result<stripe::Customer, SimplifiedStripeError> {
        error!("Called Noop Stripe Client");
        panic!("Called Noop Stripe Client");
    }

    async fn create_payment_method(
        &self, _card_number: &str, _exp_monthh: i32, _exp_year: i32, _cvc: &str,
    ) -> Result<stripe::PaymentMethod, SimplifiedStripeError> {
        error!("Called Noop Stripe Client");
        panic!("Called Noop Stripe Client");
    }
