...
pub fn calculate_work(config: &Config) -> Result<WorkCalculated, CalculateWorkError> {
    ...
    for change_description in changes {
        let metadata =
            file_metadata_repo::get(config, change_description.id).map_err(MetadataRepoError)?;
...
pub fn sync(config: &Config, f: Option<Box<dyn Fn(SyncProgress)>>) -> Result<(), SyncError> {
    let account = account_repo::get_account(config).map_err(SyncError::AccountRetrievalError)?;

    let mut sync_errors: HashMap<Uuid, WorkExecutionError> = HashMap::new();

    let mut work_calculated = calculate_work(config).map_err(SyncError::CalculateWorkError)?;
    ...
    for _ in 0..10 {
        ...
        work_calculated = calculate_work(config).map_err(SyncError::CalculateWorkError)?;
    ...
    if sync_errors.is_empty() {
        file_metadata_repo::set_last_synced(config, work_calculated.most_recent_update_from_server)
            .map_err(SyncError::MetadataUpdateError)?;
        Ok(())
    } else {
...
fn delete_file_locally(config: &Config, metadata: &FileMetadata) -> Result<(), WorkExecutionError> {
    if metadata.file_type == Document {
        // A deleted document
...
fn merge_documents(
    ...
    if local_name.ends_with(".md") || local_name.ends_with(".txt") {
        ...
        let current_version =
            file_service::read_document(config, metadata.id).map_err(ReadingCurrentVersionError)?;
