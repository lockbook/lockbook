...
struct SelectFolderNestedList<T: Identifiable & Comparable, V: View>: View {
    ...
    init(node: WithChild<T>, row: @escaping (T) -> V) {
        self.node = node
        self.row = row
        _expanded = .init(initialValue: node.level < 3)
    }
...
struct WithChild<T: Identifiable & Comparable>: Identifiable & Comparable {
    ...
    init(_ value: T, _ ts: [T], _ desc: (T, T) -> Bool, level: Int = 0) {
        self.value = value
        self.level = level
        children = ts.compactMap {
            if desc(value, $0) {
                Self($0, ts, desc, level: level + 1)
            } else {
                nil
            }
        }.sorted(by: { $0 < $1 })
