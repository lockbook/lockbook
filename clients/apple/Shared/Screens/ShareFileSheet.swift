...
struct ShareFileSheet: View {
    ...
    init(id: UUID, name: String, shares: [Share], showExitButton: Bool) {
        _model = StateObject(wrappedValue: ShareFileViewModel(id: id, name: name, shares: shares))
...
class ShareFileViewModel: ObservableObject {
    ...
    init(id: UUID, name: String, shares: [Share]) {
        ...
        self.name = name
        self.shares = shares
        readAccessUsers = shares.filter { $0.mode == .read }.map(\.with)
        writeAccessUsers = shares.filter { $0.mode == .write }.map(\.with)
    }
