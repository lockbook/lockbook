...
use lockbook_core::repo::file_metadata_repo::{FileMetadataRepo, DbError};
use lockbook_core::model::file_metadata::FileMetadata;
use lockbook_core::model::file_metadata::FileType::Folder;
use lockbook_core::service::file_service::{FileService, DocumentMoveError};
...
pub fn move_file(path1: &str, path2: &str) {
    ...
    match DefaultFileMetadataRepo::get_by_path(&db, path1) {
        Ok(maybe_old_file) => match maybe_old_file {
            ...
            Some(file1) => {
                match DefaultFileMetadataRepo::get_by_path(&db, path2) {
                    Ok(maybe_dest) => match maybe_dest {
                        Some(destination) => if destination.file_type == Folder {
                            match DefaultFileService::move_file(&db, file1.id, destination.id) {
                                Ok(_) => {},
                                Err(err) => match err {
                                    DocumentMoveError::TargetParentHasChildNamedThat => eprintln!("{} has a child named {}", path2, file1.name),
                                    ...
                                    DocumentMoveError::CouldNotFindParents(_) => eprintln!("Unexpected error: {:#?}", err),
                                },
                            }
                        } else {
