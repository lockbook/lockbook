...
impl FileTree {
    pub fn new(m: &Messenger, c: &Arc<LbCore>, hidden_cols: &Vec<String>) -> Self {
        ...
        tree_connect_key_press_event(&tree, m);
        ...
        tree_connect_drag_data_received(&tree, m, c);
        tree_connect_drag_data_get(&tree, m);
        tree_connect_drag_motion(&tree, &drag_hover_last_occurred, &drag_ends_last_occurred);
...
fn tree_connect_button_press_event(t: &gtk::TreeView, popup: &Rc<FileTreePopup>) {
    t.connect_button_press_event(glib::clone!(@strong popup => move |tree, event| {
        ...
        gtk::Inhibit(inhibit_right_click(tree, event))
    }));
}
...
fn tree_connect_key_press_event(t: &gtk::TreeView, m: &Messenger) {
    t.connect_key_press_event(glib::clone!(@strong m => move |_, key| {
...
fn tree_connect_row_activated(t: &gtk::TreeView, m: &Messenger) {
    t.connect_row_activated(glib::clone!(@strong m => move |t, path, _| {
...
fn sel_connect_changed(sel: &gtk::TreeSelection, popup: &Rc<FileTreePopup>) {
    sel.connect_changed(glib::clone!(@strong popup => move |tsel| {
                ...
                LOCKBOOK_FILES_TARGET_INFO => {
                    ...
                    for selected in paths {
                        ...
                        match c.move_file(&id, parent_id) {
                            Ok(_) => {
                                move_iter(&model, &iter, &parent, true);
                                model.remove(&iter);
                            }
...
fn move_iter(
    ...
    if let Some(it) = model.iter_children(Some(iter)) {
        move_iter(model, &it, &new_parent, false);
    }

    if !is_at_top && model.iter_next(iter) {
        move_iter(model, iter, parent, false);
    }
}

fn tree_connect_drag_data_get(t: &gtk::TreeView, m: &Messenger) {
    t.connect_drag_data_get(glib::clone!(@strong m => move |_, _, s, info, _| match info {
        LOCKBOOK_FILES_TARGET_INFO => {
        ...
        => move |w, d, x, y, time| {
            ...
            if y > height - 50 || y < 50 { // is the drag cursor 50 pixels from the top or bottom
                *drag_ends_last_occurred.borrow_mut() = Some(time);
                ...
                glib::timeout_add_local(10, glib::clone!(
                    @strong drag_ends_last_occurred,
                    @strong height,
                    @strong y
                    => move || {
