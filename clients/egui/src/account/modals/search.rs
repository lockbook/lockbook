...
impl SearchModal {
    ...
    fn show_search_result(
        ...
        let name_text =
            name_text.into_galley(ui, Some(TextWrapMode::Wrap), wrap_width, egui::TextStyle::Body);

        let path_text: egui::WidgetText = (&res.path).into();
        let path_text = path_text
            .color(ui.visuals().text_color().gamma_multiply(0.7))
            .into_galley(ui, Some(TextWrapMode::Truncate), wrap_width, egui::TextStyle::Body);
