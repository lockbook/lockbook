...
use egui::{
    Color32, DragAndDrop, Event, Id, Key, LayerId, Modifiers, Order, Pos2, Rect, Sense, Ui,
...
impl FileTree {
    ...
    fn process_input(&mut self, ui: &mut Ui, resp: &mut Response, scroll_to_cursor: &mut bool) {
        ...
        if ui.memory(|m| m.has_focus(suggested_docs_id)) {
                ...
                *scroll_to_cursor = true;
                ...
                if !collapsed_any {
                    ...
                    for &id in &self.selected {
                        ...
                        new_selection.insert(self.files.get_by_id(id).unwrap().parent);
                    }
                    self.selected = new_selection;
                    if let Some(cursor) = self.cursor {
                        let parent = self.files.get_by_id(cursor).unwrap().parent;
                        if cursor != parent {
                            selected_any_parents = true;
                        }
                        self.cursor = Some(self.files.get_by_id(cursor).unwrap().parent);
                    }
                }
            ...
            // right arrow: expand selected or move selection to first child
                ...
                *scroll_to_cursor = true;
                ...
                if !advanced_to_children {
                    ...
                    if let Some(cursor) = self.cursor {
                        let file = self.files.get_by_id(cursor).unwrap();
                        if let Some(next) = self.next(cursor, false) {
                            if self
                                .files
                                .children(file.parent)
                ...
                if !advanced_to_children && !advanced_to_siblings {
                    ...
                    for &id in &self.selected {
                        let file = self.files.get_by_id(id).unwrap();
                        let parent = self.files.get_by_id(file.parent).unwrap();
            ...
            {
                if let Some(cursor) = self.cursor {
                    let cursor_file = self.files.get_by_id(cursor).unwrap();
        ...
        if ui.memory(|m| m.has_focus(suggested_docs_id) || m.has_focus(file_tree_id)) {
            ...
            if ui.input_mut(|i| {
                ...
                for &id in self.selected.iter().chain(self.cursor.iter()) {
                    ...
                    } else if self.files.get_by_id(id).unwrap().is_document() {
                        documents.push(id);
    ...
    fn show_recursive(
        ...
        // process events before showing so the rename buffer is up-to-date when
        // show_file_cell uses its width for layout.
        let rename_id = Id::new("rename_file");
        ...
        if is_renaming {
            let rename_has_focus = ui.memory(|m| m.has_focus(rename_id));
            let rename_lost_focus =
                ui.memory(|m| m.had_focus_last_frame(rename_id)) && !rename_has_focus;
            let escape_pressed = ui.input_mut(|i| i.consume_key(Modifiers::NONE, Key::Escape));

            if !rename_has_focus && !rename_lost_focus {
                // Grant focus on the first frame of renaming; don't re-steal
                // it after the user has deliberately clicked away.
            ...
            } else if rename_lost_focus || escape_pressed {
                self.rename_target = None;
            }
            // skip drag 'n' drop / click interactions
            return resp;
        }
    ...
    fn init_rename(&mut self, file: &File) {
        ...
        if let Some(prev_sibling) = prev_sibling {
            ...
            if visible_only {
                loop {
                    ...
                    last_descendent = self.files.get_by_id(last_descendent).unwrap().parent;
                }
            }
            ...
            let mut ancestor = self.files.get_by_id(id).unwrap().parent;
            if visible_only {
                loop {
